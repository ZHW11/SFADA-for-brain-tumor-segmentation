...
class DataLoader2D(SlimDataLoaderBase):
    ...
    def generate_train_batch(self):
        ...
        for i, name in enumerate(sels):
            data = np.load(self._data[name]['path'])  # zhw
            ...
            if force_fg:
                ...
                data = data[:, sel_idx]
                # pad slice around selected location
...
def get_trainloader(config, splits_name='splits.pkl'):
    with open(os.path.join(config.DATASET.ROOT, splits_name), 'rb') as f:
